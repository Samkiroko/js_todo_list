...
export class Task {
  ...
  renderNewTask(pTaskSpace, pTaskTitle = '', id, isDone) {
    ...
    taskRow.setAttribute(
      'class',
      'form-row pb-3 taskItem d-flex justify-content-between',
    );

    taskRow.setAttribute('id', id);

    checkboxSpace.setAttribute('class', 'col-md-1');
    parentCheckbox.setAttribute(
      'class',
      'custom-control custom-checkbox myInput custom-control-indicator',
    );
    ...
    taskTitle.setAttribute(
      'class',
      'col-md-9 d-flex justify-content-md-start taskTitle',
    );
    // trash
    trashSpace.setAttribute(
      'class',
      'col-md-1 d-flex justify-content-md-end pt-1 trash',
    );
  ...
  deleteTask(element) {
    element.addEventListener('click', (e) => {
      ...
      if (
        currentElement.getAttribute('fill') === 'currentColor'
        && currentElement.parentElement.classList.contains('taskTrash')
      ) {
        for (const project of projectList) {
          for (const task of project.TaskList) {
            if (
              currentElement.parentElement.parentElement.parentElement.id
              === task.TaskId
            ) {
              return;
            }
    ...
    for (const project of projectList) {
      for (const task of project.TaskList) {
        if (task.TaskId === rewriteTaskId.textContent) {
          ...
          const tmpTaskId = document.querySelector(`#${task.TaskId}`)
            .childNodes;
  ...
  getDoneTask(element) {
    element.addEventListener('click', (e) => {
      ...
      if (currentElement.classList.contains('checkbox-1x')) {
        const currentTaskId = currentElement.parentElement.parentElement.parentElement.id;
        for (const project of projectList) {
          for (const task of project.TaskList) {
